...
try {
    ...
    if (data.online) {
        document.getElementById("player-count").textContent = `${data.players.online} Spelare uppkopplade`;
    } else {
        document.getElementById("player-count").textContent = "Servern är tillfälligt nere";
    }
} catch (error) {
...
async function fetchOnlinePlayers() {
    try {
        const response = await fetch("https://api.mcsrvstat.us/2/4qs.se");
        const data = await response.json();
